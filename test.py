...
def json_input_flow1(p, g, a, positive_prompt, negative_prompt):
    data = {
        "user_prompts": [{"prompt": p}],
        "gpt": {"gpt_prompt": config["gpt"]["prompt"]},
        "user": {"gender": g, "ages": a},
        "template": {"prompt": positive_prompt, "negative_prompt": negative_prompt},
    }
    return data
...
with st.sidebar:
    ...
    user_prompt = st.text_area(
        "写作:", placeholder="在这里模拟用户写作。如：今天风景真不错啊!", height=200
    )
...
if key == "123456":
        ...
        with st.container(border=True):
            try:
                name = st.text_input(
                    ...
                    style,
                    key="name",
                    placeholder="请命名此组提示词风格",
                )
                col1, col2 = st.columns(2)
                with col1:
                    positive = st.text_area(
                        "正向提示词",
                        style_data[style]["positive"],
                        key="positive",
                        height=200,
                        help="在这里输入正向提示词请务必预留 **{prompt}** 作为用户输入占位符",
                    )
                with col2:
                    negative = st.text_area(
                        "负向提示词",
                        style_data[style]["negative"],
                        key="negative",
                        height=200,
                        help="在这里输入负向提示词",
                    )
            ...
            st.warning(
                "正向提示词请务必预留 **{prompt}** 作为用户输入占位符；提示词命名相同时，会自动覆盖原有提示词！"
            )
            ...
            with col2:
                if st.button(
                    "保存此组提示词", use_container_width=True, type="primary"
                ):
                    if "{prompt}" in positive:
                        style_data.update(
                            {name: {"positive": positive, "negative": negative}}
                        )
                        with open(
                            f"{path}/config/style.json", "w", encoding="utf-8"
                        ) as f:
    ...
    if choose == "现有提示词":
        with st.expander(
            "**现有提示词**", expanded=True, icon=":material/construction:"
        ):
            rows = [
                [key, prompt["positive"], prompt["negative"]]
                for key, prompt in style_data.items()
            ]
            datas = pd.DataFrame(
                rows, columns=["提示词名称", "正向提示词", "负向提示词"]
            )
                ...
                if st.button("测试效果", use_container_width=True, type="primary"):
                    if user_prompt != "\n" and user_prompt is not None:
                        with st.spinner("请等待..."):
                            ...
                            url_flow2 = []
                            for i in range(num):
                                json_data = json_input_flow2(
                                    user_prompt, gender, age, style
                                )  # 发送的json数据
                                respond = get_url(json_data)  # 接收的json数据
    ...
    if choose == "所有数据":
        flow = st.selectbox(
            "选择要查看的测试数据", ["工作流1 - 调整提示词", "工作流2 - 现有提示词"]
        )
        if flow == "工作流1 - 调整提示词":
